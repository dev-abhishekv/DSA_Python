...
def get_left_right_po_subtree(post_order, left_st_len):
    # Return left and right subtree from Post-order 
    return post_order[:left_st_len], post_order[left_st_len:len(post_order)-1]
...
def buildTree(postOrder, inOrder, n) :
    ...
    # Get left and right subtree from Post-order list
    left_st_po, right_st_po = get_left_right_po_subtree(postOrder, len(left_st_io))
...
def takeInput():
    ...
    PostOrder = list(map(int, input("PostOrder: ").split()))
